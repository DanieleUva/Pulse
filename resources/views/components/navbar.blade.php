    ...
    <div class="max-w-6xl mx-auto px-6">
        <div class="flex justify-between h-20 items-center">
            ...
            <div class="hidden md:flex flex-1 max-w-md mx-8">
                <form action="{{ route('explore') }}" method="GET" class="w-full relative group">
                    <input type="text" name="search" value="{{ request('search') }}" placeholder="Cerca persone..." 
            ...
            <div class="flex items-center space-x-6">
                <a href="{{ route('explore') }}" class="font-black text-xs uppercase tracking-widest {{ request()->routeIs('explore') ? 'text-indigo-400' : 'text-slate-400' }} hover:text-indigo-400 transition">
                    Esplora
                </a>
                ...
                <div class="flex items-center space-x-2">
                    ...
                    <div class="relative p-2 text-slate-400 hover:text-indigo-400 transition cursor-pointer group">
                        ...
                        @if($unreadNotifications > 0)
                            <span class="absolute top-1 right-1 h-4 w-4 bg-indigo-500 rounded-full flex items-center justify-center text-[9px] text-white font-black shadow-[0_0_10px_#6366f1]">
                                {{ $unreadNotifications }}
                            </span>
                        @endif
                    </div>
